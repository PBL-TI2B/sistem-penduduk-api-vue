...
const onSubmit = handleSubmit(async (values) => {
    try {
        const res = await apiPost("/pekerjaan", values); // API call to create new pekerjaan

        resetForm(); // Reset form fields after submission
        toast.success("Berhasil Tambah Data Pekerjaan");
        router.visit("/pekerjaan"); // Redirect to pekerjaan list
    } catch (error) {
...
<template>
    ...
    <div class="grid gap-1">
        ...
        <BreadcrumbComponent
            :items="[ { label: 'Dashboard', href: '/' }, { label: 'Data Pekerjaan', href: '/pekerjaan' }, { label: 'Tambah Data Pekerjaan' } ]"
        />
    </div>
