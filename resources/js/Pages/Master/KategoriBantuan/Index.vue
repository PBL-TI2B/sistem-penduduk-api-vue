...
const fetchData = async () => {
    try {
        ...
        const res = await apiGet("/kategori-bantuan", { page: page.value });
...
<template>
    <Head title=" | Data Kategori Bantuan" />
    <div class="flex items-center justify-between py-3">
        <div class="grid gap-1">
            <h1 class="text-3xl font-bold">Data Kategori Bantuan</h1>
            <BreadcrumbComponent
                :items="[
                    { label: 'Dashboard', href: '/' },
                    { label: 'Data kategori bantuan' },
                ]"
            />
        </div>
        <div class="flex flex-wrap gap-4 items-center">
            <Button asChild>
                <Link :href="route('kategori-bantuan.create')"> + Kategori Bantuan</Link>
            </Button>
        </div>
    </div>
